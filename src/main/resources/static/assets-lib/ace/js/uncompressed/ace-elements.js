...
(function($ , undefined) {
	...
	Ace_File_Input.prototype.apply_settings = function() {
		...
		this.$element.after('<label class="file-label" data-title="'+this.settings.btn_choose+'"><span class="file-name" data-title="'+this.settings.no_file+'">'+(this.settings.no_icon ? '<i class="'+this.settings.no_icon+'"></i>' : '')+'</span></label>'+(remove_btn ? '<a class="remove" href="javaScript:"><i class="'+this.settings.icon_remove+'"></i></a>' : ''));
	...
	var preview_image = function(file) {
		...
		reader.onload = function (e) {
			...
			$(img).one('load', function() {
				...
				console.info(thumb);
				add_input_base64(self.$label, thumb.src);
	...
	var add_input_base64 =function (label, base64) {
		var eleId = $(label).prev().attr("id");
		var eleName = $(label).prev().attr("name");
		...
		if(!form.find("#"+eleId+"_input").attr("name")){
			var str = '<input id="'+eleId+'_input" name="'+eleName+'" type="text" style="display: none" value="'+base64+'"/>';
			form.append(str);
		}else{
			$("#"+eleId+"_input").val(base64);
		}
	};
	...
	var get_thumbnail = function(img, size, type) {
		...
		try {
			...
			dataURL = canvas.toDataURL(/*type == 'image/jpeg' ? type : 'image/png', 10*/type, 0.7);
			console.info(type);
		} catch(e) {
			dataURL = null;
		}
		...
		if(!( /^data\:image\/(png|jpe?g|gif|jpg);base64,[0-9A-Za-z\+\/\=]+$/.test(dataURL))) dataURL = null;
		if(! dataURL) return null;

		return {src: dataURL, w:w*scale, h:h*scale};
	}
