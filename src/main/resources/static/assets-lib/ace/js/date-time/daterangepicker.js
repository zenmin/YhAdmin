...
!function ($, moment) {
    ...
    var DateRangePicker = function (element, options, cb) {
        ...
        this.container.find('.calendar')
            ...
            // 添加时间选择框变化时，触发updateFormInputs事件
            .on('change.daterangepicker', 'select.hourselect,select.minuteselect,select.ampmselect', $.proxy(this.updateFormInputs, this));
    ...
    DateRangePicker.prototype = {
        ...
        setOptions: function(options, callback) {
            ...
            // 添加isInit条件，保证重新调用setOptions时，左右日期选择框不会每调用一次就互换位置
            if (this.opens == 'right' && (options.isInit == null || options.isInit)) {
        ...
        renderCalendar: function (calendar, selected, minDate, maxDate) {
            ...
            // 左翻
            var startDayOfCurMonth = calendar[1][1];
            for (var i = 0; i < 7; i++) {
                if (calendar[0][i].date() == 1) {
                    startDayOfCurMonth = calendar[0][i];
                    break;
                }
            }

            // calendar[1][1] -> startDayOfCurMonth
            if (!minDate || minDate.isBefore(startDayOfCurMonth)) {
            ...
            // 右翻
            var endDayOfCurMonth = calendar[1][1];
            var hasEnd = false;
            for (var i = 2; i < 7; i++) {
                if (calendar[4][i].date() == 1) {
                    endDayOfCurMonth = calendar[4][i - 1];
                    hasEnd = true;
                    break;
                }
            }
            ...
            if (!hasEnd) {
                for (var i = 0; i < 3; i++) {
                    if (calendar[5][i].date() == 1) {
                        if (i == 0) {
                            endDayOfCurMonth = calendar[4][6];
                        } else {
                            endDayOfCurMonth = calendar[5][i - 1];
                        }
                        break;
                    }
                }
            ...
            // calendar[1][1] -> endDayOfCurMonth
            if (!maxDate || maxDate.isAfter(endDayOfCurMonth)) {
